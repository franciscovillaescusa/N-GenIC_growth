...
static int NCDMGrowth,NBGrowth,NNUGrowth;  //number of rows in the growth files 
...
static struct pow_growth
{ 
  double k_growth,f_growth;
}
  *CDMGrowth,*BGrowth,*NUGrowth;
...
void read_growth_file(void)
{
  ...
  /* ############################################ */
  /* Read the file with the baryons growth factor */
  /*                                              */
  /* ############################################ */
  sprintf(buf, FileWithBGrowth);
  if(!(fd = fopen(buf, "r")))
    {
      printf("can't read input Growth in file '%s' on task %d\n",
	     buf,ThisTask); FatalError(19);
    }
  ...
  do
    {
      ...
      if(fscanf(fd, " %lg %lg", &k_growth, &f_growth) == 2)
	NBGrowth++;
      else
	break;
    }
  ...
  if(ThisTask == 0)
    {
      printf("found %d rows in file %s\n", NBGrowth,FileWithBGrowth);
      fflush(stdout);
    }

  BGrowth = malloc(NBGrowth * sizeof(struct pow_growth));  
  ...
  do
    {
      ...
      if(fscanf(fd, " %lg %lg", &k_growth, &f_growth) == 2)
	{
	  BGrowth[NBGrowth].k_growth = k_growth;
	  BGrowth[NBGrowth].f_growth = f_growth;
	  NBGrowth++;
	}
      else
	break;
  ...
  /* ############################################ */
  /* Read the file with the CDM growth factor     */
  /*                                              */
  /* ############################################ */
  sprintf(buf, FileWithCDMGrowth);
  if(!(fd = fopen(buf, "r")))
    {
      printf("can't read input Growth in file '%s' on task %d\n",
	     buf,ThisTask); FatalError(19);
  ...
  if(ThisTask == 0)
    {
      printf("found %d rows in file %s\n", NCDMGrowth,FileWithCDMGrowth);
      fflush(stdout);
    }

  CDMGrowth = malloc(NCDMGrowth * sizeof(struct pow_growth));  
  ...
  do
    {
      ...
      if(fscanf(fd, " %lg %lg", &k_growth, &f_growth) == 2)
	{
	  CDMGrowth[NCDMGrowth].k_growth = k_growth;
	  CDMGrowth[NCDMGrowth].f_growth = f_growth;
	  NCDMGrowth++;
	}
      else
	break;
  ...
  /* ############################################ */
  /* Read the file with the NU growth factor     */
  /*                                              */
  /* ############################################ */
  sprintf(buf, FileWithNUGrowth);
  if(!(fd = fopen(buf, "r")))
    {
      printf("can't read input Growth in file '%s' on task %d\n",
	     buf,ThisTask); FatalError(19);
  ...
  if(ThisTask == 0)
    {
      printf("found %d rows in file %s\n", NNUGrowth,FileWithNUGrowth);
      fflush(stdout);
    }

  NUGrowth = malloc(NNUGrowth * sizeof(struct pow_growth));  
  ...
  do
    {
      ...
      if(fscanf(fd, " %lg %lg", &k_growth, &f_growth) == 2)
	{
	  NUGrowth[NNUGrowth].k_growth = k_growth;
	  NUGrowth[NNUGrowth].f_growth = f_growth;
	  NNUGrowth++;
	}
      else
	break;
...
double Growth_Tabulated(double k)
{
  ...
  switch (Type)
    {
    case 0:
      growth_factor = BGrowth_Tabulated(k);
      break;
      
    case 1:
      growth_factor = CDMGrowth_Tabulated(k);
      break;
      
    case 2:
      growth_factor = NUGrowth_Tabulated(k);
      break;
  ...
  while(binhigh - binlow > 1)
    {
      ...
      if(k < BGrowth[binmid].k_growth)
	binhigh = binmid;
      else
	binlow = binmid;
    }

  dk = BGrowth[binhigh].k_growth - BGrowth[binlow].k_growth;

  if(dk == 0)
    FatalError(777);

  u = (k - BGrowth[binlow].k_growth) / dk;

  f_growthI = (1 - u) * BGrowth[binlow].f_growth + u * BGrowth[binhigh].f_growth;
...
double CDMGrowth_Tabulated(double k)
{
  ...
  if(k < CDMGrowth[0].k_growth || k > CDMGrowth[NCDMGrowth - 1].k_growth)
    return 0;

  binlow = 0;
  binhigh = NCDMGrowth - 1;

  while(binhigh - binlow > 1)
    {
      binmid = (binhigh + binlow) / 2;
      if(k < CDMGrowth[binmid].k_growth)
	binhigh = binmid;
      else
	binlow = binmid;
    }

  dk = CDMGrowth[binhigh].k_growth - CDMGrowth[binlow].k_growth;

  if(dk == 0)
    FatalError(777);

  u = (k - CDMGrowth[binlow].k_growth) / dk;

  f_growthI = (1 - u) * CDMGrowth[binlow].f_growth + u * CDMGrowth[binhigh].f_growth;
...
double NUGrowth_Tabulated(double k)
{
  ...
  if(k < NUGrowth[0].k_growth || k > NUGrowth[NNUGrowth - 1].k_growth)
    return 0;
  ...
  while(binhigh - binlow > 1)
    {
      binmid = (binhigh + binlow) / 2;
      if(k < NUGrowth[binmid].k_growth)
	binhigh = binmid;
      else
	binlow = binmid;
    }

  dk = NUGrowth[binhigh].k_growth - NUGrowth[binlow].k_growth;

  if(dk == 0)
    FatalError(777);

  u = (k - NUGrowth[binlow].k_growth) / dk;

  f_growthI = (1 - u) * NUGrowth[binlow].f_growth + u * NUGrowth[binhigh].f_growth;
...
sprintf(buf, FileWithInputSpectrum);
  ...
  do
      ...
      if(fscanf(fd, " %lg %lg %lg %lg %lg %lg %lg", &k, &T_cdm, &T_b, &dummy, &dummy, &T_nu, &T_tot) == 7)
	{
	  ...
	  if (OmegaBaryon == 0  && OmegaDM_2ndSpecies != 0)
            {
	      T_cdmnew = (0.049*T_b+(Omega-0.049-OmegaDM_2ndSpecies)*T_cdm)/(Omega-OmegaDM_2ndSpecies);
...
void initialize_powerspectrum(void)
{
  ...
  if(ReNormalizeInputSpectrum == 0 && WhichSpectrum == 2)
    {
      Type = 100;
      Norm=1.0; //Put this here to correctly compute the value of sigma8
      ...
      if (ThisTask == 0)
	printf("Normalization NOT adjusted to  Sigma8 at initial redshift read in camb=%g   (Normfac=%g)\n\n", Sigma8, Sigma8 * Sigma8 / res);
      Dplus = 1.0;
      res2 = TopHatSigma2(R8);
      if (ThisTask == 0){
	printf("Sigma8 at initial redshift via camb=%g  \n", sqrt(res2)); 
	printf("Value of Sigma8 used for calculation=%g  \n\n",Sigma8);
	  }
    }
  else
    {
...
void fermi_dirac_init_nu(void)
{
  ...
  NU_V0 = 150.9 * (1.0e5 / UnitVelocity_in_cm_per_s) * (1 + Redshift) * (1.0 / NU_PartMass_in_ev);
